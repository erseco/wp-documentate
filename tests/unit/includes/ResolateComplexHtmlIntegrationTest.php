...
class ResolateComplexHtmlIntegrationTest extends WP_UnitTestCase {
	...
	public function test_complete_document_with_all_html_types_renders() {
		...
		$term    = wp_insert_term( 'Tipo HTML Completo ' . uniqid(), 'resolate_doc_type' );
		...
		$_POST['resolate_field_body'] = wp_slash( $complex_html );
	...
	public function test_repeater_with_complex_html_renders() {
		...
		// Create document type
		$term    = wp_insert_term( 'Tipo Repetidor HTML ' . uniqid(), 'resolate_doc_type' );
		$term_id = intval( $term['term_id'] );
		update_term_meta( $term_id, 'resolate_type_template_id', $tpl_id );
